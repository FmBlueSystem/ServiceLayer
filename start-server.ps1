...
if ($nodeProcesses) {
    ...
    Write-Host ""
    Write-Host "Usa 'stop-server.ps1' para detener el servidor primero," -ForegroundColor Yellow
    Write-Host "o usa 'restart-server.ps1' para reiniciar." -ForegroundColor Yellow
    Write-Host ""
    Exit 1
}
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd C:\Projects\ServiceLayer; Write-Host 'Servidor Node.js iniciado' -ForegroundColor Green; node src/index.js" -WindowStyle Normal
...
Write-Host "==================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "El servidor esta corriendo en:" -ForegroundColor White
