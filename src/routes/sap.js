...
router.post('/items', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (result.success) {
      ...
      // Handle case where sapService returns success: false
      logger.error('SAP Items query returned failure', {
        companyDB,
        error: result.error,
        statusCode: result.statusCode,
      ...
      res.status(result.statusCode || 500).json({
        success: false,
        error: result.error || 'Failed to retrieve items',
        statusCode: result.statusCode,
        sapError: result.sapError,
        companyDB,
        timestamp: new Date().toISOString()
...
router.post('/fichas-tecnicas', asyncHandler(async (req, res) => {
  const { sessionId, filters = {}, companyDB = 'SBO_GT_STIA_PROD' } = req.body;
...
router.post('/fichas-tecnicas/create', asyncHandler(async (req, res) => {
  const { sessionId, fichaData, companyDB = 'SBO_GT_STIA_PROD' } = req.body;
...
// =================== TEST UNIT FIELDS ENDPOINT ===================

// Test endpoint to validate available unit fields in Items
router.post('/test-unit-fields', asyncHandler(async (req, res) => {
  const { sessionId, companyDB, itemCode = '' } = req.body;
  ...
  logger.info('Testing unit fields in Items', {
    companyDB,
    itemCode: itemCode || 'first available',
    requestId: req.requestId
  });
  ...
  try {
    ...
    // Get one item to see all available fields
    let endpoint = '/b1s/v1/Items?$top=1';
    if (itemCode) {
      endpoint = `/b1s/v1/Items?$filter=contains(ItemCode,'${itemCode}')&$top=1`;
    }

    const result = await sapService.callSAPAPI(endpoint, 'GET', null, headers);
    ...
    if (result.success) {
      ...
      if (firstItem) {
        // Extract all unit-related fields
        const unitFields = {};
        const allFields = Object.keys(firstItem);

        // Common SAP B1 unit field names to check
        const possibleUnitFields = [
          'InventoryUOM', 'InvntryUom', 'InventoryUoM',
          'PurchaseUnit', 'PurchaseUOM', 'BuyUnitMsr', 'PurchaseUnitOfMeasure',
          'SalesUnit', 'SalesUOM', 'SalUnitMsr', 'SalesUnitOfMeasure',
          'UoMGroupEntry', 'UoMGroupName'
        ];

        possibleUnitFields.forEach(field => {
          if (firstItem.hasOwnProperty(field)) {
            unitFields[field] = firstItem[field];
          }
        });

        logger.info('Unit fields found in Items', {
          companyDB,
        ...
        res.json({
          ...
          recommendation: Object.keys(unitFields).length > 0
            ? `Use: ${Object.keys(unitFields).join(', ')}`
            : 'No standard unit fields found - check allFieldNames for alternatives',
          timestamp: new Date().toISOString()
        });
      } else {
        res.json({
    ...
    logger.error('Test unit fields error', {
      error: error.message,
      companyDB,
      requestId: req.requestId
    });
